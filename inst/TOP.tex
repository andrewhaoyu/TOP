...
And \(\theta_{123}^{(3)}\) represent the third order interactions among
the three tumor characteristics. This decomposition is equivalent with
the first stage model. Since both the first stage and second stage have
8 parameters. We called this saturated model. The users could construct
different two-stage model by assuming different second stage parameters
to be 0. For example, the baseline only model:
\[\beta_{s_{1}s_{2}s_{3}}=\theta^{(0)}.\] This model assumes all of the
subtypes have the same log odds ratio. So it is equivalent to the
standard logistic regression. We can also construct the additive
